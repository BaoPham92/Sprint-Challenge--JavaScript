...
// Filter method
graduates.filter(index => {
  return universities.push(index.university);
});
...
zooAnimals.forEach(index => {
  return displayNames.push(
    `Name: ${index.animal_name}, Scientific: ${index.scientific_name}`
  );
});
...
// Method
zooAnimals.map(index => {
  lowCaseAnimalNames.push(index.animal_name.toLowerCase());
});
...
// Method
zooAnimals.filter(index => {
  // Stored expression
  const population = index.population < 5;
